...
class LiveSet:
    ...
    def unpack(self):
        ...
        with gzip.open(self.als_filename, 'rb') as unzipped_file:
            ...
            self.xml_filename = self.als_filename.replace(".als", "_unpacked.xml")
...
if __name__ == '__main__':
    als1 = LiveSet("test_unpacked.xml")
    als2 = LiveSet("test3_unpacked.xml")

    als1.parse("xml-test-files/test_unpacked.xml")
    als2.parse("xml-test-files/test4_unpacked.xml")

    print("Tracks in ALS1:", [track.name for track in als1.tracks])
    print("Tracks in ALS2:", [track.name for track in als2.tracks])

    for track1 in als1.tracks:
        matching_track = next((track2 for track2 in als2.tracks if track2.name == track1.name), None)
        if matching_track:
